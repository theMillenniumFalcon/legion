...
/**
 * 
 * API format: legion.com/api/v1/[:api-id]/[:path]?query=
 * 
 * @param req 
 * @param res 
 */
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const isPartialQueryEnabled = !!apiRoute.partialQuery.enabled && requestUrl.searchParams.has('legion-filter');
    ...
    if (isPartialQueryEnabled && apiResponse.headers['content-type'].includes('application/json')) {
       ...
       */
      await runMiddleware(req, res, middlewares.partialJsonQuery(requestUrl.searchParams.get('legion-filter')));
    }
