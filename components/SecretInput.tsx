...
export const SecretInput: React.FC<secretinputProps> = ({
    name, control, containerProps = {}, inputProps: { required = false, ...props }
}) => {
    ...
    return (
        ...
        >
            ...
            >
                {
                    field.value.split(/({{.*?}})/g).map((text: string, i: React.Key) => {
                        const secretMatch = text.match(/{{(.*?)}}/)
                        if (secretMatch !== null) {
                            const matchTrimmed = secretMatch[1].trim()
                            const secretExists = projectSecrets.some(({ name }) => {
                                name.trim() === matchTrimmed
                            })
